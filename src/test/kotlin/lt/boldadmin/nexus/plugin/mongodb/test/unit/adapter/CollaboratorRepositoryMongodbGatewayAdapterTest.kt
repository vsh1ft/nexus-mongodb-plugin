...
class CollaboratorRepositoryMongodbGatewayAdapterTest {
    ...
    fun `Gets collaborator by id`() {
        ...
        doReturn(Optional.of(expectedCollaborator)).`when`(collaboratorRepositorySpy).findById(COLLABORATOR_ID)
    ...
    fun `Gets collaborator by mobile number`() {
        val expectedCollaborator = createCollaborator()
        doReturn(expectedCollaborator).`when`(collaboratorRepositorySpy).findByMobileNumber(COLLABORATOR_NUMBER)
